...
<Meta title="Foundations/Content/Overview" />
...
{VOICE_VALUES.length} canonical voices, {Object.keys(voicePatterns).length} patterns authored. Browse the [Voices Overview](?path=/docs/foundations-content-voices-overview--docs) to see all 8 side-by-side with examples.
...
1. Add the slug to [`vocabularies/industry.ts`](?path=/docs/foundations-content-vocabularies--docs).
...
- [Industries index](?path=/docs/foundations-content-industries-dental--docs)
