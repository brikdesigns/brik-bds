...
<Meta title="Motion/Tiers/Lightweight" />
...
Six stagger delays available: `.bds-stagger-1` through `.bds-stagger-6`. Values come from `--stagger-1` through `--stagger-6` tokens in `animations.css`. For dynamic stagger (unknown item count, reversed sequences), see the [GSAP tier](?path=/docs/motion-tiers-gsap--docs).
...
See [Scroll Reveals](?path=/docs/motion-effects-scroll-reveals--docs) for the full effect reference.
...
See [Hover Effects](?path=/docs/motion-effects-hover--docs) for magnetic and cursor-trail variants (those require Premium JS).
...
- [Scroll Reveals (full demo set)](?path=/docs/motion-effects-scroll-reveals--docs)
