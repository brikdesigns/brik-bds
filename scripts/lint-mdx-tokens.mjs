 ...
 *
 * ── What it checks ─────────────────────────────────────────────────────────────
 * For every semantic-family token name that appears inside a **fenced code block**
 * or a **markdown table** in the docs corpus, the name must be declared in the
 * canonical registry (`dist/tokens.css`) OR be a component-scoped custom property
 * declared under `components/ui/` in a `.css` file (a documented CSS-Override-API knob). A
 ...
 *
 * ── Exit codes ─────────────────────────────────────────────────────────────────
 *   0  Clean — every documented token name resolves
 *   1  Phantom token name(s) found
...
function buildKnownTokens(tokensOverride) {
  ...
  if (tokensOverride) {
    return declaredNamesIn(readFileSync(resolve(tokensOverride), 'utf8'));
  }
  ensureDistTokens();
  const known = declaredNamesIn(readFileSync(DIST_TOKENS, 'utf8'));
...
function scanFile(file, known) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const m of line.matchAll(TOKEN_RE)) {
      ...
      violations.push({ file: rel, line: i + 1, token: tok, text: line.trim() });
    }
  }
  return violations;
}
...
function main() {
  ...
  const known = buildKnownTokens(tokensOverride);
  ...
  const violations = [];
  for (const f of files) violations.push(...scanFile(f, known));

  if (jsonMode) {
    console.log(JSON.stringify(
      { files: files.length, known: known.size, violations }, null, 2,
    ));
  ...
  if (violations.length === 0) {
    console.log(
      `lint-mdx-tokens: clean — ${files.length} MDX file(s), ${known.size} known token names, 0 phantom(s)\n`,
    );
    process.exit(0);
  }

  console.log(`\nlint-mdx-tokens: ${violations.length} phantom token name(s) in docs MDX\n`);
  const byFile = {};
  ...
  for (const [f, vs] of Object.entries(byFile)) {
    ...
    for (const v of vs.sort((a, b) => a.line - b.line)) {
      console.log(`    ${v.line}: \x1b[31m${v.token}\x1b[0m  — not in dist/tokens.css nor any component CSS`);
    }
  }
  console.log(
    '\n  A token name in a code block or table must resolve to a real token.\n' +
