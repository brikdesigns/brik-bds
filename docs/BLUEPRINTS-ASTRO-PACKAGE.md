...
2. **Portal engineering reference for `dev_scaffold_site`** → new portal doc at [content/docs/site-scaffold-task.mdx](../../../product/brik-client-portal/content/docs/site-scaffold-task.mdx) → publishes to `staging--brik-client-portal.netlify.app/docs/site-scaffold-task`. Admin-facing: inputs, failure modes, how to re-run.
