...
**Note:** the file-level refactor of existing stories to this shape is shelved
on `task/storybook-shape-migration-wip` (not merged — it conflicts with the
ADR-007 page-recipe migration that landed on `main`). The ADR captures the
decisions; new component story files should follow them from day one.
