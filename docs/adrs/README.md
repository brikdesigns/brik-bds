...
| [ADR-012](./ADR-012-link-component-escape-hatch.md) | **Accepted** (2026-06-16) | `linkComponent` escape hatch — nav-owning components (`NavItem`/`SidebarNavigation`/`SubNavigation`/`Footer`/`Breadcrumb`/`TabBar`/`NavBar`/`Menu`/`Pagination`) accept a router-aware component (Next.js/Remix `Link`) instead of emitting a bare `<a>` that forces a full-page reload; falls back to `<a>` when `disabled` or `href`-less. |
