...
| [ADR-012](./ADR-012-service-surface-inverse.md) | Proposed | Service-line surface `-inverse` — re-admits a per-service `-inverse` for the **surface** purpose only (white in light → `{hue}-darkest` in dark) for service-identified hero cards. Narrows ADR-011 §Decision pt-3; bg/text/border keep neutral inverse + `-on-*`. |
