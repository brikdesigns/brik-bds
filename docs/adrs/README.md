...
| [ADR-024](./ADR-024-regression-test-home-and-visual-gate.md) | Proposed | Regression-test home + visual cost model — behavioral assertions stay `play`-only stories (they already run in the wired `storybook` vitest project: 404/404 in 70s), sidebar noise fixed with an `interaction-test` tag rather than a `*.test.tsx` migration; visual regression moves to self-hosted Vitest `toMatchScreenshot` on free public-repo CI, Chromatic retained for hosted Storybook + MCP only, Starter ($179/mo) declined. Free-plan quota measured exhausted on every build sampled across 981→1024 (≥6 days, zero coverage). |
