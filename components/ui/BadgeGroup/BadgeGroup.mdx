...
BadgeGroup does not enforce appearance consistency — pick one appearance per cluster (`solid` or `subtle`) for visual consistency. See [Badge › Appearances](/?path=/docs/components-indicator-badge--docs) for the comparison.
