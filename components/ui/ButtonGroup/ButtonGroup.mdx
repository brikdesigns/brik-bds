...
Groups related [`Button`](/?path=/docs/components-action-button--docs) components (text or icon-only) with consistent spacing and alignment. Use `align` to control placement within the container — replaces prior manual `<Row style={{ justifyContent: ... }}>` patterns.
...
Inline toolbar of icon-only [`Button`](/?path=/docs/components-action-button--docs) children (`<Button icon={...} label="..." />`) — up to 4 actions. Common for row / card chrome (edit, copy, share, delete). The final destructive variant signals an irreversible action.
