...
Themed password field with built-in show / hide toggle. Wraps [`TextInput`](/?path=/docs/components-text-input--docs) and inherits all its props except `type` (forced internally) and `iconAfter` (reserved for the toggle button).
...
> **Note** — PasswordInput specialization is justified per [ADR-010 amendment §2](../../../docs/adrs/ADR-010-storybook-axes-of-information.md) because the visibility toggle is unique interactive behavior beyond `type="password"`. For plain text / email / other input types, use [`TextInput`](/?path=/docs/components-text-input--docs) directly with the `type` Control.
