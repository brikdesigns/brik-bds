...
Floating dropdown panel for navigation and action menus. Contains selectable items with optional icons. Closes on outside click or Escape key. Drive it with the data-first `items` API rather than composing rows by hand — `Menu` renders each row internally via the exported `MenuItem` (public for advanced/custom hosts, but not the primary path).
