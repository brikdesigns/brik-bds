...
<Canvas of={Stories.TitleWithDescription} />
<Canvas of={Stories.TitleWithEyebrow} />
<Canvas of={Stories.FullHeader} />
...
A custom `footer` always wins over mode-driven auto-footer. Use it for non-standard action sets (e.g. Archive / Cancel / Save). The mobile bottom-sheet shape is shown above under [Sides › Bottom](#sides).
...
| Primitive | Use for |
|---|---|
| [`SheetSection`](/?path=/docs/components-container-sheet-section--docs) | Named section wrapper (uppercase heading + content). One per logical grouping of fields. |
| [`Field`](/?path=/docs/components-form-field--docs) | Label + value pair. Value can be text, tags, URLs, lists — any ReactNode. |
| [`FieldGrid`](/?path=/docs/components-form-field-grid--docs) | 2 / 3 / 4-column grid of Fields or Cards. Stat-tile rows. |
| [`TagGroup`](/?path=/docs/components-indicator-tag-group--docs) | Horizontal cluster of `<Tag>` elements inside a Field. |
| [`BulletList`](/?path=/docs/components-list-bullet-list--docs) | Short-item lists (anti-messages, proof points, approved CTAs). |
| [`Card`](/?path=/docs/components-card-card--docs) / [`CardList`](/?path=/docs/components-card-card-list--docs) | Entity rows (contacts, services, scraped pages, linked records). |
| [`Table`](/?path=/docs/components-container-table--docs) | Tabular data (strategy tables, typography rows, schedules). |
| [`Accordion`](/?path=/docs/components-container-accordion--docs) | Collapsible groups when a section has many rows. |
| [`EmptyState`](/?path=/docs/components-feedback-empty-state--docs) | Whole-section "no data" treatment. Don't use per-row — `Field` has a lightweight inline empty built in. |
