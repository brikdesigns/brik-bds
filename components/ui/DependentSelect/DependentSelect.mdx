...
DependentSelect has no semantic-variant axis — the child's shape is a config choice set in code, not a story axis. Pass `child.multiple = true` for a `MultiSelect` child (shown in the Playground) or omit it for a single `Select` child.
