...
Location text input with a leading map-pin icon and an autocomplete suggestion dropdown. The component manages focus + dropdown visibility; the consumer owns the suggestions array and filtering logic. Visual treatment matches [`TextInput`](/?path=/docs/components-text-input--docs).
