...
### Where the sub-nav goes

A page-level sub-nav (`SubNavigation`) is a **full-height sibling column of `Page`**, never a child of `PageContent`: it stands beside the page body so its `bordered` divider runs the full height, with `PageHeader` + `PageContent` to its right. See the composed shell in **Layouts / app-shell → With Page**. Nesting the rail inside `PageContent` drops it below the header into the content column and breaks the two-column shell (brik-client-portal#3162). Second-level drill-down belongs in `PageHeader`'s `tabs`, not a second rail.

## Variants

### Stats in the body

Summary stat cards are a **body** concern, not a header slot — they summarize the display below them. Render them in `<PageContent>` as a `Grid` of `Card layout="metric"` above the `FilterBar` (brik-bds#404, 2026-06-03). The `PageHeader.stats` slot is deprecated.
