import { Meta, Canvas, ArgTypes } from '@storybook/addon-docs/blocks';
import { ComponentLinks } from '../../../.storybook/blocks/ComponentLinks';
import * as Stories from './PricingCard.stories';

<Meta of={Stories} />

# Pricing card

<ComponentLinks slug="pricing-card" />

> **Deprecated** — This standalone page is superseded by [Card → Pricing preset](?path=/docs/containers-card--overview). See [design.brikdesigns.com/docs/components/pricing-card](https://design.brikdesigns.com/docs/components/pricing-card) for the canonical guidance.

A pricing tier display card with title, price, features list, and call-to-action. Supports a highlighted state for the recommended plan.

## Playground

<Canvas of={Stories.Default} />

## Variants

### Highlighted

`highlighted=true` marks this tier as the recommended plan with brand surface treatment.

<Canvas of={Stories.Highlighted} />

## Props

<ArgTypes of={Stories} />
