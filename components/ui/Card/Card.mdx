...
The `mediaTreatment` prop controls how the `image` slot relates to the card edge. `flush` (default) bleeds the media to the edge and pads only the text body (`--padding-lg`) — the blog/story/product-grid look. `inset` frames the media and body together in a single `--padding-huge` inset — the service-card "card-vertical" look. One primitive serves both; `inset` replaces the former site-local `.service-card--inset` CSS override.

<Canvas of={Stories.DisplayInset} />
