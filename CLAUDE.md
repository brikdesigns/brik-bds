...
- **Visual regression** — self-hosted and live (ADR-026 Arm B, #1637). [`visual.yml`](.github/workflows/visual.yml) re-runs the `storybook` vitest project with `VISUAL_GATE=1` on every PR to `main`, diffing `toMatchScreenshot` against the baselines in `tests/visual/__screenshots__/`. A pixel change fails the check. EXPECT that on any token/CSS edit and regenerate baselines **in CI, on your branch** — run the `Update Visual Baselines` workflow (`gh workflow run update-visual-baselines.yml --ref <branch>`); NEVER commit a locally generated baseline, they are named per browser + platform and a Mac-made reference always fails on `ubuntu-latest`. The `mcr.microsoft.com/playwright` container tag is load-bearing: bump it deliberately, in lockstep with the `playwright` dependency, and regenerate baselines in the same PR.
