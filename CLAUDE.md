...
- **Chromatic = hosting, NOT a visual gate** ([ADR-026](docs/adrs/ADR-026-regression-test-home-and-visual-gate.md)). Its only job is publishing the Storybook behind the `main--` alias — the `/mcp` + manifest endpoints consumer-repo agents read. RUN `npm run chromatic` after any component CSS or story change; local Storybook is not reachable from those agents. CI publishes on a daily schedule (and `workflow_dispatch`), NOT per push, so `main--` can lag `main` by ≤24h (#1498). NEVER treat a green Chromatic check as visual coverage: a quota-exhausted build still exits 0 while capturing nothing — the only signal is `Snapshot quota reached` in the step log. The retired **`visual-review`** label is no longer wired to anything. The script resolves the token via `op run --env-file=.env.op` (a committed `op://`-ref file). On the **brik-mini** agent host this authenticates non-interactively — the `op` shim JIT-injects the `brik-claude-agent-rw` service-account token per call — so it runs **headless, with no 1Password sign-in** (#1058 resolved; verify with `op run --env-file=.env.op -- true`). On the laptop, the 1Password desktop app owns `op` identity instead (the shim is brik-mini-only). URLs + App ID: [`.claude/references/chromatic.md`](.claude/references/chromatic.md).
- **Visual regression** — self-hosted Vitest `toMatchScreenshot` in the `storybook` project (`VISUAL_GATE=1`), blocking via the [`visual.yml`](.github/workflows/visual.yml) workflow on every PR to `main` (ADR-026 Arm B, #1637; live since #1657). Baselines live in `tests/visual/__screenshots__/` — `linux`-suffixed, captured in the pinned `playwright:v1.59.1-noble` container. NEVER regenerate them locally (darwin baselines are gitignored + a false positive in CI); run the **Update Visual Baselines** workflow on your branch and the checks re-run on the pushed commit by themselves — the job pushes with a `brik-ci-bot` App installation token, which GitHub (unlike `GITHUB_TOKEN`) lets trigger CI, so the close/reopen dance #1661 prescribed is retired (#1662). The baseline diff in `git` IS the review surface (no hosted UI). Stories tagged `interaction-test` (behavioral) or `no-visual` (JS/lottie animation) are excluded. Setup + matrix: [`tests/visual/README.md`](tests/visual/README.md).
...
- **Build standards** (five composition layers: Section→Layout→Container→Block→Component; page structure + slot anatomy) → [Composition Layers](docs-site/content/docs/build-standards/composition-layers.mdx) + [Page Structure](docs-site/content/docs/build-standards/page-structure.mdx). Consult before composing any page/component; corpus: `brik-rag query "composition layers page structure" --types canon-build-standards`.
