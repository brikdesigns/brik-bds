...
|---|---|
| **Home** | `hero_fullbleed_photo` → `two_column_detail` → `testimonials_3col_cards` → `cta_split_contact` |
| **Sites & Availability** | `hero_interior_minimal` → `gallery_masonry_3col` → `cta_split_contact` |
| **Amenities** | `hero_interior_minimal` → `features_3col_icon_grid` → `gallery_masonry_3col` → `cta_centered` |
| **Rules & Policies** | `hero_interior_minimal` → `content_legal_centered` → `cta_centered` |
...
| **Our Community** (MHC) | `hero_interior_minimal` → `gallery_masonry_3col` → `story_split` → `cta_centered` |
