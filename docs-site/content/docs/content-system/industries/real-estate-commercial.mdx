...
Dental, veterinary, and optometry practices.
...
Retail, food service, professional services, and fitness.
...
|---|---|
| **Home** | `hero_split` → `services_numbered_accordion` → `testimonials_featured_large` → `cta_centered` |
| **Healthcare** | `hero_interior_minimal` → `two_column_detail` → `process_grid_4step_numbered` → `testimonials_featured_large` → `cta_split_contact` |
| **Land** | `hero_fullbleed_photo` → `story_split` → `two_column_detail` → `cta_split_contact` |
| **Commercial** | `hero_interior_minimal` → `two_column_detail` → `process_grid_4step_numbered` → `cta_centered` |
| **About** | `hero_interior_minimal` → `story_split` → `testimonials_featured_large` → `cta_split_contact` |
| **Listings** | `hero_interior_minimal` → `two_column_detail` → `cta_split_contact` |
