...
For larger illustrations and brand marks, use a static SVG asset directly. For non-icon visual primitives ([ServiceTag](/docs/components/service-tag), avatar fallbacks), use the dedicated component.
...
| `--text-positive` / `--text-negative` / `--text-status-info` | Status icons |
...
- **[Storybook icon catalog](https://storybook.brikdesigns.com/?path=/story/foundation-assets-icons--setup)**
