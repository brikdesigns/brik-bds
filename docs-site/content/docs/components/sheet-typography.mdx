...
- `<SheetSection heading="...">` — legacy `--label-sm` uppercase label-tier treatment (formerly described as "eyebrow" — banned synonym per [canon](https://design.brikdesigns.com/docs/primitives/naming-conventions#subtitle)); kept for existing consumers but new sheets should use `<SheetSectionTitle>`
...
| `detail.subHeading` | No direct replacement — uppercase label-tier subtitle is retired per the layout-contexts doc; use `<SheetSectionTitle level="h4">` for subsection hierarchy |
