...
TopNavigation is the canonical top-bar navigation for marketing sites and product landing pages. Logo on the left, links in the middle, action buttons on the right. Optional `sticky` for scroll-persistent nav.
...
```tsx
import { TopNavigation } from '@brikdesigns/bds';
```
...
- **Don't use TopNavigation for app chrome.** App shells with many sections should use [SidebarNavigation](/docs/components/sidebar-navigation) — top-bar nav with 8+ items wraps awkwardly.
- **Don't use TopNavigation for in-page section nav.** Use [TabBar](/docs/components/tab-bar).
- **Don't pair TopNavigation + SidebarNavigation as primary nav.** Pick one. Combining them makes "primary" ambiguous.
...
{/* props-check: TopNavigationProps @ components/ui/TopNavigation/TopNavigation.tsx */}
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/navigation-top-navigation--overview)**
