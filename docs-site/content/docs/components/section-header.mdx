...
<Callout type="warn">
  White on `--surface-brand-primary` is **3.78:1 — AA-large (3:1), not AA (4.5:1)**; brand-primary fills are gated AA-large by policy (see [Color Pairings](/docs/primitives/color-pairings), BDS-22 / ADR-015). The `size="lg"` title is large text; keep a band `description` short.
</Callout>
