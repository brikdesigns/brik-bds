...
<Callout type="warn">
  White on `--surface-brand-primary` is **3.78:1 — AA-large (3:1), not AA (4.5:1)**. Brand-primary fills are gated AA-large by policy (see [Color Pairings](/docs/primitives/color-pairings), BDS-22 / ADR-015). The `title` is large text and unaffected; keep a band `description` short rather than running body copy long on the fill. Long-form copy belongs on a neutral surface.
</Callout>
