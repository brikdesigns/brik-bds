...
<Callout type="warn">
  **PricingCard is deprecated as a standalone surface.** Its canonical story and guidance live with [`Card`](/docs/components/card) (the `Pricing` story in Card's Storybook file). The component file stays during the migration window for `surface-web` CSS isolation and is removed in a future major version.
</Callout>
