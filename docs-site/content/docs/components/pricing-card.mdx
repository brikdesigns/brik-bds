...
<Callout type="warn">
  **PricingCard is deprecated as a standalone surface** — but it is still the component to use for a pricing tier, and this page is its canonical guidance. [`Card`](/docs/components/card) is the intended long-term home; it has **no pricing story and no price/period/features anatomy** today (its flat slot set is `media` / `overline` / `title` / `children` / `action` / `badge`, and `CardLayout` is `'stack' | 'row' | 'metric' | 'control'`). Keep using `PricingCard` until a successor lands — tracked in [#2571](https://github.com/brikdesigns/brik-bds/issues/2571).
</Callout>
...
{/* props-check: PricingCardProps @ components/ui/PricingCard/PricingCard.tsx */}
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/deprecated-pricing-card--overview)**
