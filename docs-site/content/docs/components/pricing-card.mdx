...
<Callout type="warn">
  **PricingCard is deprecated as a standalone surface.** Its successor is [`Card` with `layout="pricing"`](/docs/components/card#pricing-layout), added in [#2571](https://github.com/brikdesigns/brik-bds/issues/2571) (ADR-038 § Amendment 2026-09-18) — same anatomy: `price`, `period`, `features`, plus the shared slots. Map `image`→`media`, `description`→`children`, `highlighted`→`variant="raised"`. The component file stays during the migration window for `surface-web` CSS isolation and is removed in a future major version.
</Callout>
