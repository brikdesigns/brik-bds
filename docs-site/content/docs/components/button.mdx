...
<ComponentPreview code={`<Button variant="negative">Delete project</Button>`}>
  <BDS.Button variant="negative">Delete project</BDS.Button>
</ComponentPreview>
...
<BDS.Button variant="outline" selected>Selected</BDS.Button>`}>
...
The full prop reference (with auto-extracted TypeScript types and live controls) lives in [Storybook → Components/Action/Button](https://storybook.brikdesigns.com/?path=/docs/components-button--overview). Summary below.
...
{/* props-check: LinkButtonProps @ components/ui/Button/LinkButton.tsx */}
| Prop | Type | Default |
|---|---|---|
...
{/* props-check: IconButtonProps @ components/ui/Button/IconButton.tsx */}
| Prop | Type | Default |
|---|---|---|
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/components-button--overview)** — every variant × size × state, live controls
- [Forms pattern](/docs/react-reference/patterns) — Save/Cancel placement, submit-state handling
