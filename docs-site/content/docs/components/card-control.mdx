---
title: Card control
description: DEPRECATED — use Card layout="control" instead.
---

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn">
  **CardControl is deprecated.** Migrate to [`Card layout="control"`](/docs/components/card#control-layout) ([#657](https://github.com/brikdesigns/brik-bds/issues/657)) — the layout carries the same layout, Badge slot, and action-alignment behavior. Per [ADR-004](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-004-component-bloat-guardrails.md), CardControl is kept during the migration window and removed in a future major version once the last consumer migrates.
</Callout>

## Migration

Same props, same defaults — move them onto `Card` and add `layout="control"`.
...
| CardControl prop | `Card layout="control"` prop |
|---|---|
| `title` | `title` |
...
The layout adds slots CardControl never had — `media` (integration logomark), `connectionStatus`, and `lastSynced` for third-party service cards. See [Card control layout](/docs/components/card#control-layout).

---

The reference below documents the frozen CardControl API for migration. New code should use [Card layout="control"](/docs/components/card#control-layout).
...
For a flexible-content card, use [Card](/docs/components/card) default mode. For stat tiles, use [Card layout="metric"](/docs/components/card#metric-layout).
...
{/* props-check: CardControlProps @ components/ui/CardControl/CardControl.tsx */}
...
- [Card](/docs/components/card) — flexible-content sibling. `Card layout="control"` is API-equivalent to CardControl.
- [CardList](/docs/components/card-list) — wrapper for stacking multiple CardControls
- [Toggle Switch](/docs/components/toggle-switch) — most common `action` content
