...
AddableEntryList is the text + textarea sibling of [AddableTagList](/docs/components/addable-tag-list). Use for lists where each row needs a headline value plus context — competitor URL + notes, reference site + why, line item + description, team member + role.
...
For free-form one-string-per-row, use [AddableTagList](/docs/components/addable-tag-list). For ≥3 structured fields, use [AddableFieldRowList](/docs/components/addable-field-row-list).
...
With `primaryStrict`, free-form entries outside the suggestion list are rejected. For full BCS-aware multi-pick with `source: 'catalog' | 'custom'` attribution, use [AddableCatalogList](/docs/components/addable-catalog-list) — it wraps AddableEntryList with extra metadata.
...
<Callout type="warn">
  **For free-form one-string-per-row, use AddableTagList.** AddableEntryList's textarea is wasted vertical space when there's no description to enter.
</Callout>

- **Don't use for ≥3 structured fields.** Use [AddableFieldRowList](/docs/components/addable-field-row-list).
- **Don't use for locked vocabularies with provenance tracking.** Use [AddableCatalogList](/docs/components/addable-catalog-list) — it wraps this component with `source: 'catalog' | 'custom'` and stable slug derivation.
...
{/* props-check: AddableEntryListProps @ components/ui/AddableEntryList/AddableEntryList.tsx */}
...
- [AddableTagList](/docs/components/addable-tag-list) — single-string sibling, with optional suggestion vocabulary
- [AddableFieldRowList](/docs/components/addable-field-row-list) — multi-field sibling
- [AddableCatalogList](/docs/components/addable-catalog-list) — wraps AddableEntryList with source attribution
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/containers-addable-entry-list--overview)**
