...
- **`subtitle`** — uppercase label-tier line above the title (entity type, parent record). Renders in `text-muted`. ("Eyebrow" is a banned synonym — see [Naming Conventions / Subtitle](https://design.brikdesigns.com/docs/primitives/naming-conventions#subtitle).)
...
- **Don't build a `ParagraphField` or `ListField`.** `Field` accepts any `ReactNode` value — paragraph-label and list-label are usages, not components. See [Naming Conventions](/docs/primitives/naming-conventions).
