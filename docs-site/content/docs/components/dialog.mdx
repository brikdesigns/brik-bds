...
**When to use:** never — Dialog is deprecated. For a confirm/cancel overlay, use [Modal `preset="confirm"`](/docs/components/modal#confirm-preset).
...
| `onConfirm` | `onConfirm` |
| `variant="negative"` | `confirmVariant="negative"` |
...
## Existing Dialog shape

For reference while migrating. New code should not use this component.

{/* props-check: DialogProps @ components/ui/Dialog/Dialog.tsx */}
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/containers-dialog--overview)** *(legacy)*
