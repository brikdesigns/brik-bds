...
`color` sets the accent fill behind the initials (`green` · `purple` · `blue` · `orange` · `yellow` · `red`, each with an AA-safe foreground) — no effect once an image loads. Consumers typically hash a stable key (e.g. contact id) to one of the six; omit for the default brand fill.

```tsx
<Avatar name="Alice Chen" color="purple" />
```
...
| Size | Diameter | Use |
|---|---|---|
| `xs` | 24px | Dense inline affordances, table cells, chips |
...
{/* props-check: AvatarProps @ components/ui/Avatar/Avatar.tsx */}
| Prop | Type | Default |
|---|---|---|
| `src` | `string` | — |
| `alt` | `string` | — |
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/foundation-assets-avatar--overview)**
