...
`ProgressStepperProps` is a discriminated union of two shapes, so `variant` selects
which of `steps` / `count` is required. The shared props first:

{/* props-check: CommonProgressStepperProps @ components/ui/ProgressStepper/ProgressStepper.tsx */}
| Prop | Type | Default |
|---|---|---|
| `activeStep` | `number` *(required)* | — |
| `size` | `ProgressStepperSize` | `'md'` |
| `linear` | `boolean` | `false` |
| `onStepClick` | `(index: number) => void` | — |

Then one of the two variant shapes:

{/* props-check: StepsProgressStepperProps @ components/ui/ProgressStepper/ProgressStepper.tsx */}
| Prop | Type | Default |
|---|---|---|
| `variant` | `'steps'` | `'steps'` |
| `steps` | `ProgressStep[]` *(required)* | — |

{/* props-check: DotsProgressStepperProps @ components/ui/ProgressStepper/ProgressStepper.tsx */}
| Prop | Type | Default |
|---|---|---|
| `variant` | `'dots'` *(required)* | — |
| `count` | `number` *(required)* | — |

`activeStep` is zero-indexed. `steps` and `count` are mutually exclusive — the union
makes passing the wrong one a type error rather than a runtime surprise.
...
- [ProgressCircle](/docs/components/progress-circle) — single-value circular progress
