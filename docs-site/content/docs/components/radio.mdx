...
For ≥6 options, use [Select](/docs/components/select). For boolean (single yes/no), use [Checkbox](/docs/components/checkbox) or [Switch](/docs/components/switch).
...
- **Don't use Radio for boolean.** "Yes / No" with two radios is awkward; use a [Checkbox](/docs/components/checkbox) for opt-in or [Switch](/docs/components/switch) for a setting.
