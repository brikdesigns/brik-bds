...
For ≥6 options, use [Select](/docs/components/select). For boolean (single yes/no), use [Checkbox](/docs/components/checkbox) or [Toggle Switch](/docs/components/toggle-switch).
...
- **Don't use Radio for boolean.** "Yes / No" with two radios is awkward; use a [Checkbox](/docs/components/checkbox) for opt-in or [Toggle Switch](/docs/components/toggle-switch) for a setting.
...
{/* props-check: RadioProps @ components/ui/Radio/Radio.tsx */}
...
- [SegmentedControl](/docs/components/segmented-control) — horizontal alternative for ≤4 short-label options
