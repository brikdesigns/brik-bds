...
- **Don't use Select for ≤3 options.** Use [Radio](/docs/components/radio) (vertical) or [SegmentedControl](/docs/components/segmented-control) (horizontal) — both surface choices visibly.
...
{/* props-check: SelectProps @ components/ui/Select/Select.tsx */}
...
|---|---|---|
| `--bds-select-chevron-color` | `var(--text-muted)` | Chevron icon color |
| `--bds-select-icon-color` | `var(--text-muted)` | Leading icon color |

```css
.branded-form {
  --bds-select-chevron-color: var(--text-brand-primary);
  --bds-select-icon-color: var(--text-brand-primary);
}
```
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/components-select--overview)**
