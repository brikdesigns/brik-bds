...
| Prop | Type | Default |
|---|---|---|
| `category` | `'brand' \| 'marketing' \| 'information' \| 'product' \| 'back-office' \| 'service'` *(required)* | — |
| `variant` | `'text' \| 'icon-text' \| 'icon'` | `'text'` |
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` |
| `label` | `string` | category default label |
| `serviceName` | `string` | — |

`category="service"` is a deprecated alias of `'back-office'`. Plus standard `<span>` HTML attributes.
