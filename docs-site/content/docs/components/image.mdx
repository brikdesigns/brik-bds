...
<ComparisonGrid
  items={[
    {
      title: 'Image',
      whenToUse: 'A content or editorial image needing SEO + <figure> semantics, alt text, and a locked shape.',
      whenNotToUse: 'A bare aspect box with no <img> (Frame), or person identity with initials (Avatar).',
      code: `<Image src="…" alt="…" ratio="16-9" />`,
    },
    {
      title: 'Frame',
      whenToUse: 'A raw aspect-ratio box for any media, with no <img>/SEO semantics of its own.',
      whenNotToUse: 'The media is a real content image that needs alt text and <figure> — that\'s Image.',
      code: `<Frame ratio="16-9">{media}</Frame>`,
    },
    {
      title: 'Avatar',
      whenToUse: 'A person\'s identity — headshot with an initials fallback.',
      whenNotToUse: 'A general content or editorial image — that\'s Image.',
      code: `<Avatar src="…" name="…" />`,
    },
  ]}
/>
...
For avatars use [Avatar](/docs/components/avatar); for a raw aspect box with no `<img>` semantics use [Frame](/docs/components/frame).
...
{/* props-check: ImageProps @ components/ui/Image/Image.tsx */}
| Prop | Type | Default |
|---|---|---|
| `src` | `string` *(required)* | — |
| `alt` | `string` *(required)* | — |
...
Plus standard `<figure>` HTML attributes. `position` takes a CSS `object-position`
value (`"top"`, `"50% 25%"`) and only applies together with `ratio` and a cropping
`fit`. `ratio` and `fit` are the same aliases [Frame](/docs/components/frame) uses —
`FrameRatio` includes four deprecated mode-words listed there. `customRatio`
takes any CSS `aspect-ratio` string and, like `ratio`, wraps the image in a
`Frame`; it overrides `ratio` for genuine one-offs that don't fit a slug. `anchor` is also
passed through to `Frame`: the default `'width'` fills the container and derives
height from the ratio; `'height'` fixes a caller-set height and derives the width
(for fixed-height thumbnails).

## Related

- [Frame](/docs/components/frame) — the bare aspect-ratio box Image wraps; use it directly for non-`<img>` media
- [Avatar](/docs/components/avatar) — person identity with initials fallback- [Color pairings](/docs/foundation/color-pairings) — for captions and overlays on images
