...
Per [ADR-005](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-005-addable-field-row-list.md). Replaces three hand-rolled patterns identified in the 2026-Q2 portal addable survey (PR #251).
...
<Callout type="warn">
  **Don't use AddableFieldRowList for single-string-per-item.** Use [AddableTagList](/docs/components/addable-tag-list) (with or without a `suggestions` vocabulary). The render-prop overhead is wasted on a single field.
</Callout>
...
{/* props-check: AddableFieldRowListProps @ components/ui/AddableFieldRowList/AddableFieldRowList.tsx */}
| Prop | Type | Default |
|---|---|---|
| `values` | `T[]` *(required)* | — |
...
- [AddableTagList](/docs/components/addable-tag-list) — single string per row, with optional suggestion vocabulary
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/containers-addable-field-row-list--overview)**
