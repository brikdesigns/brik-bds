...
```tsx
import { ServiceTagSelect } from '@brikdesigns/bds';
```
...
<Callout type="warn">
  **Don't use ServiceTagSelect for non-service categories.** It is bound to the Brik service lines. For a generic single choice, use [SegmentedControl](/docs/components/segmented-control) or [Select](/docs/components/select).
</Callout>
...
{/* props-check: ServiceTagSelectProps @ components/ui/ServiceTagSelect/ServiceTagSelect.tsx */}
