...
{/* props-check: TabBarProps @ components/ui/TabBar/TabBar.tsx */}
...
This contract is opt-in. A plain `TabBar` is label navigation — every tab is tabbable, no arrow keys. When items carry `controls` (the id of the `tabpanel` each tab drives), the rail becomes a full [WAI-ARIA tabs widget](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/): exactly one tab in the page Tab sequence (roving tabindex), and Arrow / Home / End move focus and activate — Left/Right when horizontal, Up/Down when `orientation="vertical"`. Rails without `controls` are unchanged. [MediaTabs](/docs/components/media-tabs) is the primary consumer.
...
interface TabItem {
  ...
  /** Stable id for the tab button — set alongside `controls` when wiring to tabpanels. */
  id?: string;
  /** id of the tabpanel this tab controls — emits aria-controls and enables the tabs keyboard contract. */
  controls?: string;
}
```
...
- [Accordion](/docs/components/accordion) — the ordered-sequence sibling; an accordion when the set is steps to walk through, not options to pick between
