...
<Callout type="warn">
  **AddableTextList is deprecated.** Migrate to [`AddableTagList`](https://storybook.brikdesigns.com/?path=/story/containers-addable-tag-list--default) per [ADR-003](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-003-addable-list-family.md) — one API covers plain text lists and suggestion-backed combobox lists. `<AddableTextList values={v} onChange={o} />` becomes `<AddableTagList values={v} onChange={o} />`. Kept during the migration window, removed in a future major version.
</Callout>
...
{/* props-check: AddableTextListProps @ components/ui/AddableTextList/AddableTextList.tsx */}
| Prop | Type | Default |
|---|---|---|
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/containers-addable-text-list--overview)**
