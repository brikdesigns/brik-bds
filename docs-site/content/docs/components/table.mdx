...
Table is a composable data-display primitive. The container plus its subcomponents (`TableHeader`, `TableBody`, `TableRow`, `TableHead`, `TableCell`, `TableActionsCell`, `TableSubheader`) handle the structural HTML; the consumer composes cells with whatever content the row needs — text, badges, links, action buttons. Three typed media cells (`TableAvatarCell`, `TableImageCell`, `TableServiceTagCell`) package the common identity / logo / classification layouts so consumers stop hand-rolling them.
...
Three typed cells package the recurring media layouts. Each composes an existing primitive and owns only its own layout (alignment, gap, fixed sizing) — reach for these instead of hand-rolling flex wrappers inside a plain `<TableCell>`.
...
**`TableServiceTagCell`** — hosts one or more [ServiceTag](https://storybook.brikdesigns.com/?path=/docs/components-service-tag--overview)s, wrapping with a consistent gap. Supply `variant="icon"` for icon-only classification or `variant="icon-text"` for a labeled tag.
