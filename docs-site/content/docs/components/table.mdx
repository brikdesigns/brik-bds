...
Table is a composable data-display primitive. The container plus its subcomponents (`TableHeader`, `TableBody`, `TableRow`, `TableHead`, `TableCell`, `TableActionsCell`, `TableSubheader`) handle the structural HTML; the consumer composes cells with whatever content the row needs — text, badges, links, action buttons. Typed cells (`TableIconCell`, `TableTextCell`, `TableAvatarCell`, `TableImageCell`, `TableServiceTagCell`) package the common icon / identity / logo / classification layouts so consumers stop hand-rolling them — see [ADR-027](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-027-table-cell-layer-ownership.md) for which layer owns a new cell.
...
For card-like rows where each entry is its own self-contained block, use [CardList](/docs/components/card-list). For settings panels with action-per-row, use [Card `preset="control"`](/docs/components/card#control-preset).
...
Typed cells package the recurring cell layouts. Each renders the `<td>` itself, composes an existing primitive, and owns only cell-level concerns (alignment, gap, fixed sizing, row-density padding) — reach for these instead of hand-rolling flex wrappers inside a plain `<TableCell>`. [ADR-027](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-027-table-cell-layer-ownership.md) is the rule for which layer owns a new cell: it earns a component when it renders the table element *and* owns a cell-level concern. Anything else is a primitive inside `<TableCell>`.

**`TableIconCell`** — a label preceded by a fixed 24px icon box, so labels align down the column even when glyph widths differ. The icon is `aria-hidden` — the label carries the accessible text.

```tsx
<TableIconCell icon={<Icon icon="ph:palette" />}>Design</TableIconCell>
```

**`TableTextCell`** — a two-line primary / secondary stack; the avatar-less counterpart to `TableAvatarCell`, sharing its type ramp so the two read identically in one table.

```tsx
<TableTextCell primary={user.name} secondary={user.email} />
```

**`TableAvatarCell`** — an [Avatar](/docs/components/avatar) paired with a name and an optional secondary line (email / role). Initials render when `src` is omitted; pass `primary` as a `<TextLink>` when the name should open a read sheet.
...
**`TableImageCell`** — a fixed square (1:1) thumbnail for a logo or product image, via the [Image](/docs/components/image) primitive at `ratio="1-1"`. Use `fit="contain"` for logos (no crop) and `fit="cover"` for product photos. The cell renders only — the upload / edit interaction is consumer-side (e.g. a file uploader in the edit sheet).

```tsx
<TableImageCell src={org.logoUrl} alt={`${org.name} logo`} fit="contain" size="sm" />
```

**`TableServiceTagCell`** — hosts one or more [ServiceTag](/docs/components/service-tag)s, wrapping with a consistent gap. Supply `variant="icon"` for icon-only classification or `variant="icon-text"` for a labeled tag.

```tsx
<TableServiceTagCell>
  <ServiceTag category="brand" variant="icon" />
  <ServiceTag category="marketing" variant="icon-text" serviceName="Custom Standard Web Development and Design" />
</TableServiceTagCell>
```
...
- **Don't use Table for settings rows.** Use [Card `preset="control"`](/docs/components/card#control-preset) — locked layout for badge + title + description + action.
...
{/* props-check: TableProps @ components/ui/Table/Table.tsx */}
...
| `headerBorderWeight` | `TableHeaderBorderWeight` | `'md'` |
| `roundedTop` | `boolean` | `true` |
| `roundedBottom` | `boolean` | `true` |
| `headerBackground` | `TableHeaderBackground` | `'secondary'` |
...
| `TableHeader` | `<thead>` | — |
| `TableBody` | `<tbody>` | — |
| `TableRow` | `<tr>` | `selected: boolean` — **never `onClick`** ([see callout](#cell-level-interactivity)) |
...
| `TableAvatarCell` | `<td>` | `src`, `name`, `primary`, `secondary`, `size: 'sm' \| 'md'`, `status` — Avatar + identity |
| `TableTextCell` | `<td>` | `primary` *(req)*, `secondary` — two-line identity stack, no avatar |
| `TableIconCell` | `<td>` | `icon` *(req)*, `children` *(req)* — fixed-box leading icon + label |
| `TableImageCell` | `<td>` | `src` *(req)*, `alt` *(req)*, `size: 'sm' \| 'md'`, `fit: 'contain' \| 'cover'` — square 1:1 thumbnail |
| `TableServiceTagCell` | `<td>` | `children` — one or more `ServiceTag`s |
...
- [Card `preset="control"`](/docs/components/card#control-preset) — settings-row pattern (replaces the deprecated `CardControl`)
