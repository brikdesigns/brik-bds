...
For card-like rows where each entry is its own self-contained block, use [CardList](/docs/components/card-list). For settings panels with action-per-row, use [Card `layout="control"`](/docs/components/card#control-layout).
...
- **Don't use Table for settings rows.** Use [Card `layout="control"`](/docs/components/card#control-layout) — locked layout for media + title + description + action.
...
- [Card `layout="control"`](/docs/components/card#control-layout) — settings-row pattern (replaces the deprecated `CardControl`)
