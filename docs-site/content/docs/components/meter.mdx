...
<Meter value={4} max={7} tone="warning" label="Fair" />
<Meter value={2} max={7} tone="negative" label="Fail" />
<Meter value={5} max={7} tone="neutral" />
```
...
```tsx
<Meter value={45} max={100} tone="warning" showValue={false} />
```
...
- **Don't use Meter for binary states.** Active/inactive is a [Badge](/docs/components/badge); on/off is a [Toggle Switch](/docs/components/toggle-switch).
...
{/* props-check: MeterProps @ components/ui/Meter/Meter.tsx */}
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/components-meter--overview)**
