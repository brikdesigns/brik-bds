...
For top-bar navigation in marketing sites, use [TopNavigation](/docs/components/top-navigation). For tab-style section navigation within a page, use [TabBar](/docs/components/tab-bar).
...
- Any layout where ≥5 nav items would overflow a horizontal TopNavigation
...
### With footer actions and profile
...
<Callout type="warn">
  **Don't use SidebarNavigation for marketing sites.** Use [TopNavigation](/docs/components/top-navigation) — top-bar nav matches what visitors expect on a website. Sidebars are for apps where users spend extended time.
</Callout>

- **Don't use SidebarNavigation for ≤3 nav items.** Use [TopNavigation](/docs/components/top-navigation) — a tall sidebar with 2 items reads as wasted space.
- **Don't pair SidebarNavigation with TopNavigation in the same layout.** Pick one primary nav surface; secondary nav goes inside the main content area as TabBar / Breadcrumb.
...
{/* props-check: SidebarNavigationProps @ components/ui/SidebarNavigation/SidebarNavigation.tsx */}
| Prop | Type | Default |
|---|---|---|
...
| `width` | `string` (CSS length) | `'260px'` expanded / `'80px'` collapsed |
| `linkComponent` | `BdsLinkComponent` | — |
| `userSection` | `ReactNode` | *deprecated — use `profile`; if both are passed, `profile` wins* |
...
- [TopNavigation](/docs/components/top-navigation) — top-bar alternative for marketing sites
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/navigation-sidebar-navigation--overview)**
