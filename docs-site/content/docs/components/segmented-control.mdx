...
- **Don't use SegmentedControl for binary on/off.** Use [Toggle Switch](/docs/components/toggle-switch) — the affordance is correct.
...
- Renders a `<div role="radiogroup">` with each segment as `<button role="radio">` — the correct pattern for a single-select, mutually-exclusive control.
- The selected segment carries `aria-checked="true"`; unselected carry `false`.
- Keyboard arrows move selection between segments, with `Home` / `End` and roving `tabindex` so only the active segment is in the tab order.

## API

{/* props-check: SegmentedControlProps @ components/ui/SegmentedControl/SegmentedControl.tsx */}
...
- [Toggle Switch](/docs/components/toggle-switch) — binary on/off
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/components-segmented-control--overview)**
