  ...
  for (const slug of slugs) {
    setConsoleState((prev) => ({
      ...
      items: prev.items.map((i) =>
        i.id === slug ? { ...i, status: 'in_progress' } : i
      ),
    }));
    ...
    setConsoleState((prev) => ({
      ...prev,
      items: prev.items.map((i) =>
        i.id === slug ? { ...i, status: 'completed' } : i
      ),
    }));
  }
}
...
- Per-item state changes are announced as items move from pending → in_progress → completed.
- The collapse / dismiss buttons carry `aria-label`s.

## API

{/* props-check: TaskConsoleProps @ components/ui/TaskConsole/TaskConsole.tsx */}
| Prop | Type | Default |
|---|---|---|
| `title` | `ReactNode` *(required)* | — |
| `items` | `TaskConsoleItem[]` *(required)* | — |
| `isOpen` | `boolean` | `true` |
| `position` | `TaskConsolePosition` | `'bottom-right'` |
| `onDismiss` | `() => void` | — |
| `defaultCollapsed` | `boolean` | `false` |
| `autoDismissDelay` | `number` | `0` |
| `subtitle` | `ReactNode` | — |

`autoDismissDelay` is in milliseconds; `0` means no auto-dismiss, and it only fires
once every item is done and `onDismiss` is supplied.

The `TaskConsoleItem` shape is defined alongside the component: `{ id, label, status: 'pending' \| 'in_progress' \| 'completed' \| 'failed', detail?, href? }`. The `status` string stays snake_case for API stability; only the CSS BEM modifier kebab-cases it (`in_progress` → `--in-progress`).
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/story/containers-task-console--default)**
