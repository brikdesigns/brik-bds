...
[ServiceTag](/docs/components/service-tag) renders to the left of the title for service-line pages.
...
The step names are offset from the tokens they render. `lg` is the page-title default, so it renders `--heading-xl` — the [Section-headline step](/docs/build-standards/headings) a page's `<h1>` gets. Only `md` and `sm` name-match their token.
...
{/* props-check: PageHeaderProps @ components/ui/PageHeader/PageHeader.tsx */}
...
| `subtitle` | `string` | — |
| `media` | `ReactNode` | — |
| `badge` | `ReactNode` | *deprecated* — alias for `media` (#1705) |
...
- [ServiceTag](/docs/components/service-tag) — common `media` prop content (variant="icon")
