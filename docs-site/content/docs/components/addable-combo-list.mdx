...
description: DEPRECATED — use AddableTagList instead.
---

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn">
  **AddableComboList is deprecated.** Migrate to [`AddableTagList`](https://storybook.brikdesigns.com/?path=/story/containers-addable-tag-list--default) per [ADR-003](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-003-addable-list-family.md) — one API covers plain text lists and suggestion-backed combobox lists. `<AddableComboList values={v} onChange={o} suggestions={s} />` becomes `<AddableTagList values={v} onChange={o} suggestions={s} />`; `maxEntries` is renamed `maxItems`. Kept during the migration window, removed in a future major version.
</Callout>
...
Free-form entry backed by suggestions.
...
{/* props-check: AddableComboListProps @ components/ui/AddableComboList/AddableComboList.tsx */}
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/containers-addable-combo-list--overview)**
