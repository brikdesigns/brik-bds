...
{/* props-check: TagProps @ components/ui/Tag/Tag.tsx */}
| Prop | Type | Default |
|---|---|---|
| `size` | `TagSize` | `'md'` |
...
| `children` | `ReactNode` | — |
| `onRemove` | `() => void` | — |
| `density` | `TagDensity` | `'comfortable'` |

`onRemove` is deprecated and will be removed — use [Chip](/docs/components/chip),
which owns the removable case. It is listed plainly rather than struck through so
the props gate can verify it against source; strikethrough made the row unparseable.
...
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/components-tag--overview)**
