...
Footer is the canonical site-wide footer for marketing sites and product landing pages. Logo + tagline on the left, navigation columns in the middle, copyright + social on the bottom bar. Pairs with [TopNavigation](/docs/components/top-navigation) at the top of the page.
...
{/* props-check: FooterProps @ components/ui/Footer/Footer.tsx */}
| Prop | Type | Default |
|---|---|---|
| `logo` | `ReactNode` | — |
| `tagline` | `string` | — |
...
| `columnHeadingLevel` | `FooterColumnHeadingLevel` | `'h4'` |
...
```ts
// Takes `title` (canonical) or `heading` (deprecated), never both.
type FooterColumn = { links: { label: string; href: string }[] } & (
  | { title: string; heading?: never }
  /** @deprecated Renamed to `title`; removed in the next major. */
  | { heading: string; title?: never }
);
```

`heading` is a banned slot word under the content-group taxonomy — the canonical
name for a primary text slot is `title`, matching `ContentBlock`. Existing callers
passing `heading` keep working and render identically; migrate at your convenience.

It is a union rather than two optional fields so a column still *must* be labelled:
`{ links: [...] }` with neither key would typecheck against a both-optional
interface and render a blank heading.

## Related

- [TopNavigation](/docs/components/top-navigation) — top-of-page partner
- [TextLink](/docs/components/text-link) — alternative to FooterColumn for inline link lists
- **[Storybook playground](https://storybook.brikdesigns.com/?path=/docs/navigation-footer--overview)**
