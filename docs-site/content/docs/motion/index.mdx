...
|---|---|---|
| `tokens/gap-fills.css` | Token | All tiers — the semantic durations (`--duration-sm` / `-normal` / `-slow`) and all four `--ease-*` curves (part of the `bds-tokens` bundle) |
...
- [Motion tokens](/docs/foundation/motion) — the `--duration-*`, `--ease-*`, and `--stagger-*` scales, and why the numeric duration suffix is a step index rather than a millisecond value
