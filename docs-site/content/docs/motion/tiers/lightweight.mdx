...
Six stagger delays available: `.bds-stagger-1` through `.bds-stagger-6`. Values come from `--stagger-1` through `--stagger-6` tokens in `gap-fills.css`. For dynamic stagger (unknown item count, reversed sequences), see the [GSAP tier](/docs/motion/tiers/gsap).
...
See [Hover Effects](/docs/motion/effects/hover) for magnetic and cursor-spotlight variants (those require Premium).
