...
- [Token Anatomy](/docs/foundation/token-anatomy) — the four-tier model and the naming formula these roles are composed from (the color-modifier vocabulary — `on-color`, `-on-light`/`-on-dark`, `-inverse`, tone — is documented above under [Color modifiers](#color-modifiers))
