...
<Callout type="warn">
  **The numeric suffix is a step index, not a millisecond value.** `--duration-400` is `500ms`, not `400ms`. The suffix tracks milliseconds up to `--duration-300` and then stops. Reach for `--duration-sm` / `--duration-md` / `--duration-lg` instead — they name the intent and cannot be misread.
</Callout>
...
|---|---|---|
| `--duration-sm` | `100ms` | Micro-interactions — hover, focus |
| `--duration-md` | `200ms` | Standard transitions — buttons, cards |
| `--duration-lg` | `300ms` | Emphasis — modals, sheets, reveals |
| `--duration-xl` | `500ms` | Cross-fades, long reveals — one rung past `--duration-lg` |
...
| `--duration-sm` / `-normal` / `-slow`, all `--ease-*` | `tokens/gap-fills.css` | Hand-authored |
