...
- **Text _on_ the brand fill** (`--text-on-color-dark` on `--background-`/`--surface-brand-primary`) — **always white, normal weight, every size including body copy.** 3.78:1 is owner-accepted debt ([ADR-015 § Amendment 2026-09-12](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-015-brand-primary-aa-large-contrast-policy.md)). Do **not** "fix" it by darkening the text to `--text-primary` or bolding body copy to reach large-text — both are aesthetically wrong for the brand and are disallowed. Keep band copy short; long body copy belongs on a neutral surface.
- **Small body copy in the _brand color_** — **never** `--text-brand-primary`; use `--text-primary`. This is the OTHER case (Poppy-colored text on a neutral surface, not white-on-brand). Formerly advisory (the gate scores token pairings, not rendered font sizes), now CI-asserted by `npm run lint-brand-text`, which flags `--text-brand-primary` on `--body-*`-sized text ([#1064](https://github.com/brikdesigns/brik-bds/issues/1064)).
...
| Rail TOC active label on surface | AA ≥4.5 | 6.23:1 AA | 5.55:1 AA |

| Rail TOC resting label on surface | AA ≥4.5 | 12.63:1 AAA | 15.91:1 AAA |
...
| Service brand — text on pale surface | AAA ≥7 | 8.4:1 AAA | 7.87:1 AAA |
...
| Service product — text on mid-tone surface | AA ≥4.5 | 6.06:1 AA | 6.06:1 AA |
...
| Service information — inverse card text (dark) | AA ≥4.5 | — | 9.29:1 AAA |
...
| Body text on the yellow accent tint | AAA ≥7 | 15.77:1 AAA | 19.22:1 AAA |

| Body text on the blue accent tint | AAA ≥7 | 12.47:1 AAA | 17.7:1 AAA |

| Body text on the green accent tint | AAA ≥7 | 16.25:1 AAA | 20.59:1 AAA |

| Body text on the purple accent tint | AAA ≥7 | 11.72:1 AAA | 14.29:1 AAA |

| Body text on the orange accent tint | AAA ≥7 | 11.95:1 AAA | 17.83:1 AAA |
