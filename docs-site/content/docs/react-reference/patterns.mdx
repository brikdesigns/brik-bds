...
| **Sheet-driven edit** | Editing one record from a list | Wrap in `<Sheet>` with the [`useSheetStack`](/docs/react-reference/hooks#usesheetstack--read--control-the-sheet-navigation-stack) hook for back/save flow |
| **Multi-step wizard** | Onboarding, contracts | Tabs in the sheet config (see [Hooks → useConfigureSheet](/docs/react-reference/hooks#useconfiguresheet--register-a-headless-sheet-config)) |
