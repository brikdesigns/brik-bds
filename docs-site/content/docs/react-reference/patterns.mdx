...
Patterns are recipes, not new components. If a recipe gets used in 3+ places without meaningful variation, it graduates to a real component (see [Utilities → Adopt / Extend / Graduate](/docs/react-reference/utilities#adopt-extend-graduate)).
...
| **Sheet-driven edit** | Editing one record from a list | Wrap in `<Sheet>` with the [`useSheetStack`](/docs/react-reference/hooks#usesheetstack) hook for back/save flow |
| **Multi-step wizard** | Onboarding, contracts | Tabs in the sheet config (see [Hooks → useConfigureSheet](/docs/react-reference/hooks#useconfiguresheet)) |
...
**Layout chrome lives once, at app root** — `<TopNavigation>` + `<SidebarNavigation>` + `<BrikDevBar>` (dev only). Templates render *inside* that chrome via Next.js layout files. Don't re-mount the chrome inside a route.

## Navigation

Four navigation surfaces, four components — keep them straight. They are not interchangeable, and they don't stack past one level each.

| Surface | Component | When |
| --- | --- | --- |
| **Top app bar** | `<TopNavigation>` | Global brand + primary destinations + user menu |
| **Section sidebar** | `<SidebarNavigation>` | Within an app section (Settings, Admin) — collapsible groups |
| **Second-column section nav** | `<SubNavigation>` | Section-specific links beside a collapsed `<SidebarNavigation>`, one column per shell |
| **Within-page** | `<Breadcrumb>` + `<Pagination>` | Position + page-by-page traversal |

Max depth is one `<SubNavigation>` per shell. Drill down further with `<PageHeader>`'s `tabs` (TabBar) or inline sections in the content column — never a second `<SubNavigation>`.
...
**Severity color rides on the icon, not the surface.** A status `<Banner>` sits on `--surface-secondary` in every tone; the signal is its leading status `<Badge>`, which reads `--background-positive`, `--background-warning`, `--background-negative`, or `--background-status-info` for its own fill. Pass `tone` and both are wired for you — you never pick a hex, and you never set the banner's background to carry severity.
...
- [Hooks](/docs/react-reference/hooks) — `useSheetStack` and `useConfigureSheet` power the sheet-driven recipes here
