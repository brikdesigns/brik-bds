...
**Severity color rides on the icon, not the surface.** A status `<Banner>` sits on `--surface-secondary` in every tone; the signal is its leading status `<Badge>`, which reads `--background-positive`, `--background-warning`, `--background-negative`, or `--background-info` for its own fill. Pass `tone` and both are wired for you — you never pick a hex, and you never set the banner's background to carry severity.
