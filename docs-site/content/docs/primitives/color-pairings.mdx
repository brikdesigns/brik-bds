...
<Callout type="info">
  **The AA-passing near-Poppy step now exists — and is not wired up yet.** `--color-poppy-600` (`#c94428`) hits **4.84:1** on white: it clears AA-normal while staying visibly Poppy, which is exactly what ADR-015 and ADR-016 recorded as unavailable. It shipped with the [11-step scale](/docs/primitives/color#the-11-step-scale) in v0.152.0, unblocking ADR-016's Option C.
