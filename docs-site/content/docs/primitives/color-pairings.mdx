...
The gate runs on pre-push (`validate:full` → `scripts/validate-all.js`), in CI on every PR (`contrast-gate.yml`), at release (pre-publish), and in `pr-checklist.sh`. The CI check runs on every PR but is not yet a required status check — merge safety rests on merge-wait discipline:
