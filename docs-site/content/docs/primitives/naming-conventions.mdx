...
4. **Variant values come from the canon axis matrix.** `variant ∈ {primary, secondary}`, `appearance ∈ {solid, subtle, outline}`. Don't invent `variant="persona-cluster"` or `variant="dark-centered"`.
