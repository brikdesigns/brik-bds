...
These slots are drift that still ships. They are in the registry today and are **renames owed**, tracked on [#1910](https://github.com/brikdesigns/brik-bds/issues/1910). Do not reference them in new work.

{/* lint-mdx-tokens-ignore-start — this table is the retired/drift list by definition */}

| Shipping name | Why it's drift | Use instead |
|---|---|---|
| `--web`, `--tablet`, `--mobile` | No slot at all — the exact shape the callout above retires. Unitless duplicates, zero `var()` references in `dist/` or `components/`. | `--breakpoint-web` / `-tablet` / `-mobile` |
...
- [Composition Layers](/docs/build-standards/composition-layers) — the pedagogical Section → Layout → Container → Block → Component model that composes tokenized primitives
