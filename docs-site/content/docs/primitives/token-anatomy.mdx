...
A token references a **lower tier** by default. One exception: a **derived** token may reference **another token at its own tier** when that reference is the mechanism by which it inherits the referenced token's [Mode](#mode) modulation — and only then.
