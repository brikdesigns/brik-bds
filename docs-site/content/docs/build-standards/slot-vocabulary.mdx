...
Every term here matches the canonical BEM pattern enforced by `scripts/slot-pattern-check.mjs`. It is a curated reference of the roles the system already names — reach for it before coining a new slot, and reuse an existing role rather than invent a synonym. See [Naming Principles](/docs/build-standards/naming-principles) for the pattern rule.
...
Title and section-title **copy** uses title case: capitalize the first, last, and every major word; lowercase short articles, coordinating conjunctions, and prepositions of three letters or fewer; always capitalize pronouns. Full rule, examples, and scope guards live in [Headings → Casing](/docs/build-standards/headings#casing). This governs heading copy only — never CMS-authored names or conversational status microcopy.

## Subtitle

The secondary line paired with a title. In a section-heading context it is always `subtitle`. Never `eyebrow`.

- BEM: `.bds-*__subtitle`
- Rendered as: `<p>` by default
- Used in: `PageHeader`, `Sheet`, `SheetSection`, `BoardCard`, `DataSection`

`eyebrow`, `kicker`, and `pre-title` are common marketing terms. They are not BDS terms. Translate to `subtitle` before writing code.

<Callout type="info">
  `overline` is the one exception, and it is not a synonym for `subtitle`. It is a canonical Card anatomy slot — the short label *above* the title, carrying a `Tag` / `ServiceTag` / category mark, or the label above a `layout="metric"` value. See [ADR-038](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-038-card-anatomy-over-preset-union.md) and [Card](/docs/components/card). BEM: `.bds-card__stack-overline`, `.bds-card__row-overline`. Outside Card anatomy, `overline` remains banned — use `subtitle`.
</Callout>
...
|---|---|---|
| Pull-quote with attribution | `Testimonial` | `__quote` / `__quote-cite` / `__quote-text` |
...
- The component rendering two buttons → `ButtonGroup` (but in a **width-constrained data-grid cell**, keep a non-wrapping `<Stack direction="horizontal" justify="end">` — ButtonGroup wraps + forces `gap-md`, see [Button group → When *not* to use](/docs/components/button-group))
...
- [Naming Principles](/docs/build-standards/naming-principles) — the slot pattern and how to name a new slot
- [HTML Semantics](/docs/build-standards/html-semantics) — which HTML element each slot renders as
- [`docs/SLOT-ALLOWLIST.md`](https://github.com/brikdesigns/brik-bds/blob/main/docs/SLOT-ALLOWLIST.md) — the pattern spec (grammar + examples)
