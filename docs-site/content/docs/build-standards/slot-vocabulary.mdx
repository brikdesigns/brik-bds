...
`Field` handles the inline vs block distinction internally — put any `ReactNode` as `children`. `CardSummary` renders values as heading-tier typography (stat tile, not a form field) — don't confuse the two.
