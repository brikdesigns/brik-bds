...
<Callout type="warn">
  Never use `--gap-xs` or `--gap-tiny` for content rhythm — both collapse to `0px` outside the default [spacing mode](/docs/primitives/spacing#spacing-modes), which would erase the tight step. And never hardcode a px value: the scale re-modulates per mode, a literal does not.
</Callout>
