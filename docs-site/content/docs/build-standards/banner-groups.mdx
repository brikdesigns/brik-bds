...
<Callout type="warn">
  **`announcement` is last, unconditionally** — and it does not belong in a group that also carries an `negative`. A promotion sitting above, beside, or below a payment failure reads as the system not knowing what state the reader is in. Suppress the announcement while any `negative` banner is live.
</Callout>
