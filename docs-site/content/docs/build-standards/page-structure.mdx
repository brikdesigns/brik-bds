...
A section is a `<section>` element carrying a blueprint BEM block name. The valid block names are defined in [Naming Principles](/docs/build-standards/naming-principles#blueprint-naming-rules): `bds-hero`, `bds-cta`, `bds-services`, `bds-features`, `bds-about`, `bds-callout-panel`.
...
Sections span full viewport width. Content is constrained by an inner container slot — this is CSS, not a wrapper element with a `.container` class. The recipe, default band (`--content-width-xl`), width roles, and the `--page-inset` are locked in [Page Grid](/docs/build-standards/page-grid).
