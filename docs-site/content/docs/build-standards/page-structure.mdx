...
Both components use only mode-tied `--gap-*` tokens — never raw px, never `--gap-xs`/`--gap-tiny` (both resolve to `0px` outside default mode, which would collapse the tight gap).

| Component | Relationship | Token |
|---|---|---|
| `ContentBlock` | `title` → `subtitle` | `--gap-sm` |
| `ContentBlock` | `title`/`subtitle` → `description`, and last text slot → `actions` | `--gap-md` |
| `Prose` | `:is(h1,h2,h3) + p` | `--gap-md` |
| `Prose` | `p + p` | `--gap-lg` |

Block-to-block spacing — the gap between one `ContentBlock`/`Prose` and the next — is unchanged: it's the surrounding section container's existing `--gap-xl`, not something either component owns.
