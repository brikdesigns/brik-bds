...
Every slot name matches the canonical [slot pattern](/docs/build-standards/naming-principles#slot-pattern). The common slots inside a section:
...
Use [Layout primitives](/docs/build-standards/composition-layers#the-layers) — `Stack`, `Cluster`, `Grid`, `Split`, `Row` — to arrange blocks inside `__content`. Do not hand-roll flex/grid in section CSS when a layout primitive covers the case.
...
## Content blocks and prose

Text inside `__content` (or any Block-layer slot) renders through one of two Block components rather than raw `<h2>`/`<p>` — each owns the vertical rhythm its shape needs. See [ADR-023](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-023-content-blocks-and-prose-rhythm.md).

- **`ContentBlock`** — a fixed-slot unit (`title` / `subtitle` / `description` / `actions`, each omittable) for named content with a known shape: a card's text, a feature's copy, a CTA's heading + button.
- **`Prose`** — free-form, already-sanitized CMS HTML (formalizes the legacy `.rich-content` pattern) for open-ended rich text with no fixed slot shape.

Fixed slots and free prose need different spacing mechanisms — named-slot adjacency vs. element adjacency — so one component owns each, rather than a `mode` prop hiding two behaviors behind one name.

```tsx
import { ContentBlock, Prose } from '@brikdesigns/bds';
...
Both components draw their slot and element rhythm from the same role→step scale — `title` → `subtitle` is `--gap-sm`, heading/subtitle → body is `--gap-md`, paragraph → paragraph is `--gap-lg` — set with adjacency margin so each pair lands its exact step. Block-to-block spacing between one `ContentBlock`/`Prose` and the next is `--gap-xl`, owned by the section's layout, not by either component. The full scale and the gap-vs-padding-vs-margin rule live in [Content Rhythm](/docs/build-standards/content-rhythm).

<Callout type="info">
  `title` element level follows outline position, same as any other `__title` slot — see [HTML Semantics — Heading element selection](/docs/build-standards/html-semantics#heading-element-selection). `ContentBlock` defaults `titleAs` to `h3` (the common case nests inside a `Card`); pass `h2` when it's a direct outline sibling of the page's `<h1>`.
</Callout>

## Container widths

Sections span full viewport width. Content is constrained by an inner container slot — this is CSS, not a wrapper element with a `.container` class. The recipe, default band (`--content-width-xl`), width roles, and the `--gutter-page` inset are locked in [Page Grid](/docs/build-standards/page-grid).
...
- [Page Archetypes](/docs/build-standards/page-archetypes) — the four product page shapes that fill the app shell above
...
- [Naming Principles](/docs/build-standards/naming-principles) — blueprint naming rules and the slot pattern
