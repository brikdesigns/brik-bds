...
description: Six conceptual layers — Blueprint, Layout, Container, Block, Control, Component — and the decision rules for assembling them.
---
...
The one-sentence form: *Blueprints compose the page. Containers hold collections. Blocks fill containers. Layouts arrange. Components are atoms.*

## The layers

| Layer | Responsibility | Vocabulary | Examples |
|---|---|---|---|
| **Blueprint** | Page role with surrounding structure (vertical rhythm, container, background surface) | Page-level semantic | `Hero`, `Content`, `CTA` |
...
<Callout type="info">
  **"Blueprint" is the layer; "section" is not.** Three different things shared the word `Section` before [ADR-037](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-037-astro-is-the-canonical-blueprint-rail.md) §5 renamed this layer, and the other two are unchanged:

  - `section_type` — a field on every `blueprints/blueprint-library.json` entry (`hero`, `services`, `cta`, …). It says which page slot a blueprint fills, not which layer it is.
  - `bds-blueprint-section` / `bds-blueprint-section__container` — the shared section-shell class ([ADR-021](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-021-blueprint-section-shell.md)), and the `<section>` element it renders.

  Renaming either would break the manifest or the BEM canon. Only the composition layer moved.
</Callout>
...
|---|---|---|
| Blueprint | `content-system/blueprints/{react,astro}/` | Blueprint families per ADR-008 play this layer |
| Layout | `components/ui/` | `Stack`, `Cluster`, `Grid`, `Split`, `Row` |
| Container | `components/ui/` | `Card`, `Accordion`, `List` |
| Block | `components/ui/` | `Field`, `Card layout="metric"` |
| Control | `components/ui/` | `FilterBar`, `PageHeader`, `TabBar`, `SubNavigation`, `Pagination` |
| Component | `components/ui/` | Atomic primitives |

<Callout type="info">
  The Blueprint layer is parameterized along four curated axes — structure (`layout`, `align`, `columns`, `ratio`), `media`, motion (`reveal`, `contentMotion`), and style (the Theming Dimensions) — so one blueprint spans many client looks without hand-rolled CSS. Every value is a closed enum an agent selects, never free config. See [ADR-039](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-039-blueprint-parameterization-axes.md).
</Callout>
...
When a change names an element ("the card", "the section", "the button"), find the target by its **layer**, reading the DOM top-down (Blueprint → Layout → Container → Block or Control → Component) — **never by selector-name resemblance.**

A BEM **block name** identifies the blueprint family; it does **not** define the element's layer. A block whose name contains "card" can still be a **Blueprint**. The Card is whichever nested element plays the **Container** role.

<Callout type="warn">
  `bp-hero-img-card` is a **Blueprint** (rendered as a `<section>`). The **Card** is its nested Container, `aside.bp-hero-img-card__media-card`. They own different surfaces: the **Blueprint** carries the page-role/band surface; the **Container (Card)** carries its own bounded surface (its `--bds-hero-img-card-media-bg` hook). A surface meant for a service-identified card — e.g. ADR-020's `--surface-service-{line}-inverse` — is a **Container** surface; applying it to the Blueprint repaints the whole band, not the card. (Regression caught in brikdesigns#637.)
</Callout>
...
- [Page Archetypes](/docs/build-standards/page-archetypes) — the four assembled product pages these layers compose into
- [Theming — Blueprints](/docs/theming/blueprints) — the blueprint families that make up this layer
