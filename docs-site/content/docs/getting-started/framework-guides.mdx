...
|---|---|---|
| **Next.js (App Router)** | Product apps — the client portal | [`brik-client-portal`](https://github.com/brikdesigns/brik-client-portal) |
...
BDS does not ship fonts. Load them via `next/font` in `src/lib/fonts.ts` and expose CSS variables that the BDS `--font-family-*` tokens can consume:
...
Don't reach for `var(--text-primary)` directly in TSX — wrap tokens in typed primitives (`src/lib/tokens.ts` + `src/lib/styles.ts`) so editor autocomplete catches typos at write-time. Full pattern: [React composition layer](/docs/getting-started/react-composition).
...
Vite (Astro's bundler) lets you `import` CSS files. Unlike a CSS `@import`, a JS `import` **can't** carry a `layer(…)` clause — so Astro's cascade is governed by **import order**, not by `@layer`. Order them so BDS canon loads first, then your client overrides last (last wins):

```astro
---
// src/layouts/BaseLayout.astro

// Cascade by source order (JS imports can't assign a layer): BDS canon → client overrides last.
...
### 5. Theme and mode switches

Astro is HTML — there's no `ThemeProvider`. Set switches as attributes on the `<html>` element directly. Full attribute table in [The cascade → HTML attribute switchboard](/docs/getting-started/cascade#html-attribute-switchboard).
...
Full pattern: [Client Themes → Per-audience scope binding](/docs/theming/client-themes#per-audience-scope-binding).

### 7. Atmospheres and blueprints
...
```astro
---
import Hero from '@brikdesigns/bds/blueprints-astro/Hero.astro';
---

{/* A block is one file; the layout it renders in is a prop. */}
<Hero layout="interior-minimal" section={section} clientFacts={clientFacts} theme={theme} />
```
...
- [Theming](/docs/theming) — the four Theming Dimensions (tokens, atmospheres, archetypes, blueprints)
