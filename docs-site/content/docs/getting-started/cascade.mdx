...
BDS resolves tokens through a strict cascade: **Libraries** define values, **Tiers** stack abstractions, **Layers** order the CSS @layer at runtime, and **Modes** vary values on orthogonal axes. Every consumer (portal, brikdesigns, every client site) follows the same path. Six words for six independent concepts — see [Token Anatomy](/docs/primitives/token-anatomy) for the full disambiguation map.
