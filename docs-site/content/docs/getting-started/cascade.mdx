...
BDS resolves tokens through a strict cascade: **Libraries** define values, **Tiers** stack abstractions, **Layers** order the CSS @layer at runtime, and **Modes** vary values on orthogonal axes. Every consumer (portal, brikdesigns, every client site) follows the same path. Six words for six independent concepts — see [Token Anatomy](/docs/foundation/token-anatomy) for the full disambiguation map.
...
| `modes-borderwidth.css`, `modes-spacing.css` | `bds-tokens` | Mode mode overrides (see [Modes](#modes) below) |
...
| **Blueprints** | Section composition — hero / services / testimonials shapes | Authored in BDS, tagged by mood + industry; rendered on the Astro rail ([ADR-037](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-037-astro-is-the-canonical-blueprint-rail.md)), React for CMS-backed sites |
...
Blueprint **keys name layout only** — `card_grid`, `hero_split`, `cta_centered`. Content domain lives in the library entry's `industries` / `moods`, brand and atmosphere in tokens ([ADR-037](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-037-astro-is-the-canonical-blueprint-rail.md) §4). See [Blueprints](/docs/theming/blueprints) for the two-rail relationship.
...
Full registry at [Color](/docs/foundation/color). Canonical bundle: `dist/tokens.css`.

<Callout type="warn">
  **Names not in the registry are not real.** Inventing a `--surface-paper`, `--text-on-ink`, `-soft` / `-warm` modifier, or a `--theme-brik-*` / `--brand-{tier}` namespace creates parallel taxonomy drift. If you need a new token, add it to a Library first via Figma → Tokens Studio → Style Dictionary. See [Token Anatomy](/docs/foundation/token-anatomy) for the closed forms.
</Callout>
...
| `data-audience` / `data-service` / `data-department` | Per-site convention | Re-binds canonical brand tokens within a subtree (multi-brand pattern — see [Client Themes → scope binding](/docs/theming/client-themes#per-audience-scope-binding)) |
...
This page is the **canonical adoption contract**: every consumer (portal, brikdesigns, every client site) wires the cascade the same way, and [Installation](/docs/getting-started/installation) links here as the one setup path. A consumer is compliant when all four hold:

1. **Declare the layer order** — `@layer bds-tokens, bds-components, client-theme, client-overrides;` before any imports ([CSS layers](#css-layers)).
...
3. **Import each BDS file into its layer** — `@import '@brikdesigns/bds/tokens.css' layer(bds-tokens);` and `styles.css layer(bds-components);`. `tokens.css` ships **unlayered**, so the `layer()` clause is what places it — without it the file wins as unlayered CSS and the client theme can never override. Never the internal bundle files directly, never a hand-rolled copy of a scale. A bare `layer` (no name) fails too: an anonymous layer can't be named in a `@layer` statement, so it can't be ordered against `client-theme`. Enforced by the `import-layer` rule in [`cascade-contract-check`](#where-the-cascade-is-enforced). (Astro/Vite JS `import`s can't carry `layer()`; those consumers cascade by source order instead — see [Framework guides](/docs/getting-started/framework-guides) — and the rule skips them by construction.)
4. **Hold zero token redefinitions** — see the rule below.

<Callout type="warn">
  **Consumers consume tokens; they do not redefine them.** A consumer `:root` / theme / override block may **not** redefine a BDS-owned scale or semantic token — `--heading-*`, `--display-*`, `--font-size-*`, `--surface-*`, `--text-*`, `--border-*`, `--page-*`, `--background-*`. Redefining a *real* token (e.g. `--heading-lg: var(--font-size-700)`) is distinct from inventing one: it passes the "names not in the registry" check above yet still forks the system and drifts silently (the brikdesigns scaffold heading remap was load-bearing and invisible for ~3 months). **The one exception is brand-color override** via a Brand Kit `theme-{client}.css` in the `client-theme` Layer — that's the [client-theme](#what-lives-in-the-client-theme-layer) job. Need a value the scale doesn't have? Add it to a Library (Figma → Tokens Studio → Style Dictionary) or emit the [Mode](#modes) that carries it — never redefine in the consumer. See [ADR-013](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-013-token-last-mile-enforce-contract-complete-emission.md).
</Callout>
...
| `@brikdesigns/bds/cascade-contract-check` (consumer dep) | Three rules: `no-redefinition` (clause 4 above), `typography-family` (font-family family matches font-size family within a rule), `import-layer` (clause 3 above — every CSS `@import` of `tokens.css` / `styles.css` carries a named `layer()`) | BDS-owned export, imported by consumer `lint-tokens`. Live in brikdesigns; portal wiring tracked in [#1437](https://github.com/brikdesigns/brik-bds/issues/1437) |
| `npm run cascade-contract-check` | The same engine pointed at this docs site's own `app/global.css` — the site that publishes the contract is itself a consumer of it | BDS CI + `npm run validate` |

Run BDS gates locally: `npm run validate`. Consumer gates: `./scripts/token-audit.sh --canonical-only`.

The three rules split by what they can see. `canonical-check` answers "does this name exist"; `cascade-contract-check` answers "is this name allowed *here*" — a redefinition passes the first and fails the second. `import-layer` is the only one of the three that reads a statement rather than a declaration, which is why a non-compliant import went unnoticed: nothing was looking above the first `{`. See [ADR-013 §2](https://github.com/brikdesigns/brik-bds/blob/main/docs/adrs/ADR-013-token-last-mile-enforce-contract-complete-emission.md).

## Related

- [Token Anatomy](/docs/foundation/token-anatomy) — the six-concept disambiguation map + Tier abstraction stack
