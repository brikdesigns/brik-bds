...
3. **Import each BDS file into its layer** — `@import '@brikdesigns/bds/tokens.css' layer(bds-tokens);` and `styles.css layer(bds-components);`. `tokens.css` ships **unlayered**, so the `layer()` clause is what places it — without it the file wins as unlayered CSS and the client theme can never override. Never the internal bundle files directly, never a hand-rolled copy of a scale. (Astro/Vite JS `import`s can't carry `layer()`; those consumers cascade by source order instead — see [Framework Guides](/docs/getting-started/framework-guides).)
