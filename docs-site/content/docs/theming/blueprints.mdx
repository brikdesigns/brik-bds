...
|---|---|---|---|
| `<Hero>` | `split` | `hero_split_60_40` | `hero` |
| `<Hero>` | `interior-minimal` | `hero_interior_minimal` | `hero` |
| `<Hero>` | `with-pricing-card` | `hero_split_image_card_overlay` | `hero` |
| `<CardGrid>` | `card-grid` | `services_3col_card_grid` | `services` |
| `<CardGrid>` | `two-column-list` | `services_detail_two_column` | `services` |
| `<Cta>` | `split` | `cta_split_contact` | `cta` |
| `<Cta>` | `default` | `cta_dark_centered` | `cta` |
| `<About>` | — | `about_story_split` | `about` |
| `<Features>` | — | `features_3col_branded_dark` | `features` |
| `<CalloutPanel>` | — | `support_plan_callout_split` | `support_plan` |
...
5. After merge: publish the new BDS version and regenerate the portal seed migration via `npx tsx scripts/generate-blueprint-seed.mts` in `brik-client-portal`. If keys were **removed** from the library, the regenerated `00114` seed does not delete their rows — an already-applied migration is not re-run, so a removal needs its own `DELETE` migration.
