...
| The per-hue accent set (`--text/background/surface-accent-{hue}`) | **No** | All three purposes are Figma-sourced from the system color primitives, so `accent-purple` is the same purple in every theme. Category color, not a valence — see [The per-hue accent set](/docs/primitives/color#the-per-hue-accent-set). |
