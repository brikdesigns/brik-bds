...
Each `.body.theme-{name}` block overrides a fixed set of **semantic color tokens** — the same tokens documented in [Primitives → Color](/docs/foundation/color). These include:
...
:root {
  /* Brand primitives — one ramp per client hue, follows the Figma 6-stop rail
     ...
     /docs/foundation/color → "Tonal scale + interaction state". */
...
| The `info` signal (`--background-info`, `--surface-info`, `--text-info`) | **No** | Blue, universal semantic meaning. Gap-fills over the system blue primitive. |
| The per-hue accent set (`--text/background/border/surface/page-accent-{hue}`) | **No** | Mirrored 1:1 from the ❖ Brik Brand Kit Figma library and Figma-sourced from the system color primitives, so `accent-purple` is the same purple in every theme. Category color, not a valence. `red` is the one exception — it aliases the brand poppy — see [The per-hue accent set](/docs/foundation/color#the-per-hue-accent-set). |
...
- [Primitives → Color](/docs/foundation/color) — the semantic tokens this layer overrides
