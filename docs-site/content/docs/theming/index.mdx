...
<Callout type="info">
  **Audience:** primarily websites (Astro; Webflow as a legacy surface) and any product surface that needs per-client branding. Foundation tokens are shared across product and websites; theming is where the per-client divergence happens.
</Callout>
...
<Callout type="warn">
  **"Dimension" not "Layer".** Older docs called these four "Layers" — confusing because CSS `@layer` is also called Layer (the `bds-tokens` / `client-theme` cascade). They're distinct concepts: a Theming Dimension is a *what to decide*; a CSS Layer is a *where the decision's CSS lands*. See [Token Anatomy](/docs/foundation/token-anatomy) for the full six-concept disambiguation.
</Callout>
...
Full override matrix and the minimal `theme-{client}.css` template: [Client Themes](/docs/theming/client-themes#overridability-matrix).
...
- [Foundation / Primitives](/docs/foundation) — the token layer the brand theme overrides
